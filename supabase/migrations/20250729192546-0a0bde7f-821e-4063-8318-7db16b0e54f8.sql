-- Atualizar a função de envio de email de boas-vindas para usar a nova Edge Function
...
BEGIN
  ...
  IF NEW.ativo = true AND NEW.email != 'admin@teste.com' AND is_auth_user THEN
    ...
    -- Log inicial
    RAISE NOTICE 'Enviando email de boas-vindas para usuário: % (%) - Função: %', 
                 NEW.nome, NEW.email, NEW.funcao;
    ...
    BEGIN
      ...
      IF http_response.status = 200 THEN
        RAISE NOTICE 'Email de boas-vindas enviado com sucesso para: % (%) - Senha padrão: Trocar@123', NEW.nome, NEW.email;
      ELSE
